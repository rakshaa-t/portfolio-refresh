...
export const PortfolioMobile: React.FC<PortfolioMobileProps> = (props: PortfolioMobileProps) => {
  ...
  return (
    <div style={{width: '100%', minHeight: '100vh', position: 'relative', background: '#E3DDED', overflow: 'hidden'}}>
      ...
      {/* Header Bar */}
      <div style={{width: '100%', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(227, 221, 237, 0.95)', backdropFilter: 'blur(10px)'}}>
        <div style={{textAlign: 'center', color: 'white', fontSize: 36, fontFamily: 'Neulis Cursive', fontWeight: '500', wordWrap: 'break-word'}}>raks</div>
        <div style={{display: 'flex', alignItems: 'center', gap: 16, opacity: 0.44}}>
          <a href="https://linkedin.com/in/raksha-tated" target="_blank" rel="noopener noreferrer" style={{width: 29, height: 29, display: 'flex'}}>
          ...
          <a href="https://twitter.com/raksha_tated" target="_blank" rel="noopener noreferrer" style={{width: 24, height: 24, display: 'flex'}}>
        ...
        {PROJECT_CARDS.map((card) => {
          ...
          return (
            <div
              ...
              style={{width: 263, height: 266.05, position: 'relative', background: 'rgba(255, 255, 255, 0.30)', borderRadius: 44, outline: '1px white solid', outlineOffset: '-1px', cursor: 'pointer'}}
            >
              <img style={{width: 263, height: 216.05, left: 0, top: 50, position: 'absolute', borderRadius: 44, outline: '1px white solid', objectFit: 'cover'}} src={card.image} alt={card.title} />
              <div style={{position: 'absolute', left: 25, top: 22, color: 'black', fontSize: 14, fontFamily: 'Nexa', fontWeight: '400', wordWrap: 'break-word'}}>
                {card.title}
              </div>
            </div>
          );
      ...
      <div style={{width: '100%', position: 'fixed', bottom: 0, left: 0, right: 0, padding: '16px 20px', background: 'rgba(227, 221, 237, 0.95)', backdropFilter: 'blur(10px)', zIndex: 50, boxSizing: 'border-box'}}>
        <div style={{width: '100%', maxWidth: '280px', margin: '0 auto', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12}}>
          {/* Chat Button (Active) */}
          <button style={{flex: 1, height: 60, padding: 12, position: 'relative', background: '#283FE4', boxShadow: '1px 2px 4px rgba(0, 0, 0, 0.10)', borderRadius: 9999, outline: '1px white solid', display: 'flex', justifyContent: 'center', alignItems: 'center', border: 'none', cursor: 'pointer'}}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="7" stroke="white" strokeWidth="2" />
            </svg>
          </button>
          {/* Inbox Button */}
          <button style={{flex: 1, height: 60, padding: 12, background: 'rgba(255, 255, 255, 0.64)', borderRadius: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center', border: 'none', cursor: 'pointer'}}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="3" y="6" width="18" height="12" rx="2" stroke="#575757" strokeWidth="2" />
            </svg>
          </button>
          {/* Calendar Button */}
          <button
            onClick={() => window.open('https://cal.com/raksha-tated-v2ee58/15min', '_blank')}
            style={{flex: 1, height: 60, padding: 12, background: 'rgba(255, 255, 255, 0.64)', borderRadius: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center', border: 'none', cursor: 'pointer'}}
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="3" y="4" width="18" height="18" rx="2" stroke="#575757" strokeWidth="2" />
              <line x1="8" y1="2" x2="8" y2="6" stroke="#575757" strokeWidth="2" strokeLinecap="round" />
              <line x1="16" y1="2" x2="16" y2="6" stroke="#575757" strokeWidth="2" strokeLinecap="round" />
              <line x1="3" y1="10" x2="21" y2="10" stroke="#575757" strokeWidth="2" />
            </svg>
          </button>
        </div>
      </div>
